...
export default function useBackgroundImage(
  ...
  const backgroundImage = useTransform(localProgress, (latest) => {
    ...
    for (let i = 0; i < divisions; i++) {
      temp += `#2ec4b6 ${i * (vh / divisions)}vh ,#2ec4b6 ${func(i, latest) * (vh / divisions) + i * (vh / divisions)}vh,rgba(0,0,0,0) ${func(i, latest) * (vh / divisions) + i * (vh / divisions)}vh,rgba(0,0,0,0) ${(i + 1) * (vh / divisions)}vh`;
